...
BOOL DoFileOpenSave(HWND hwnd, BOOL bSave) {
	...
	ofn.hwndOwner = hwnd;
	ofn.lpstrFilter = "文本文档 (*.txt)\0*.txt\0全部文件 (*.*)\0*.*\0\0";
	...
	if(bSave) {
		...
		if(GetOpenFileName(&ofn)) {
			if(!LoadFile(GetDlgItem(hwnd, IDC_MAIN_TEXT), szFileName)) {
				MessageBox(hwnd, "加载文件失败", "错误！",MB_OK|MB_ICONEXCLAMATION);
				return FALSE;
			}
		}
	}
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case CM_FILE_EXIT:
					...
					break;
				case CM_ABOUT:
					MessageBox (NULL, "这是由 FOCA 推出的清爽，无捆绑的记事本" , "关于我们", 0);
					break;
			}
			break;
		case WM_CLOSE:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow) {
	...
	if(!RegisterClassEx(&wc)) {
		MessageBox(0,"窗口出错","错误！",MB_ICONEXCLAMATION|MB_OK|MB_SYSTEMMODAL);
		return 0;
	}

	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE,"WindowClass","FOCA 记事本",WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		500,500,
